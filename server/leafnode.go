...
func (s *Server) createLeafNode(conn net.Conn, rURL *url.URL, remote *leafNodeCfg, ws *websocket) *client {
	...
	c.ws = ws
	if rURL != nil {
		// For remote, check if the scheme starts with "ws", if so, consider
		// that this is an attempt to connect using websocket.
		if isWSURL(rURL) {
...
func (c *client) leafNodeSolicitWSConnection(opts *Options, rURL *url.URL, remote *leafNodeCfg) ([]byte, ClosedState, error) {
	...
	if err == nil &&
		(resp.StatusCode != 101 ||
			...
			resp.Header.Get("Sec-Websocket-Accept") != wsComputeAcceptKey(wsKey)) {
	...
	if err == nil && c.ws.compress {
		...
		if ext := resp.Header.Get("Sec-WebSocket-Extensions"); ext == _EMPTY_ ||
			!strings.Contains(ext, "permessage-deflate;") {
...
func wsComputeAcceptKey(wsKey string) string {
	h := sha1.New()
	h.Write([]byte(wsKey))
	h.Write(wsGUID)
